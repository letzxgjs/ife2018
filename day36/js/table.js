...
function renderTableAndChart () {
	...
	if (datas) {
		for (var i = 0, len = datas.length; i < len; i++) {
			saleDataOnly.push(datas[i].sale)
		}
		var max = getMaxValue(saleDataOnly)
		var colors = ['#FF0000','#0094FF','#B200FF','#4CFF00','#FF7FED','#FF006E','#404040','#DAFF7F','#FF006E']
		for (var i = 0, len = datas.length; i < len; i++) {
			createLineChart(datas[i].sale, colors[i])
		}
	}
}
...
function getMaxValue(data) {
	return max = data.reduce(function (x, y) {
		if (x instanceof Array) { x = getMaxValue(x) }
		if (y instanceof Array) { y = getMaxValue(y) }
		return (x > y ? x : y) 
	})
}
// console.log('max', getMaxValue([122,2,3,[1254,5,[1456,[12354,12390]],1116],[9,10],[111,125]]))
